<?php

namespace App\Filters;

use Illuminate\Support\Str;
use App\Enums\OrderByDirectionEnum;
use App\Enums\EnquiriesListOrderByFieldsEnum;

class EnquiriesOrderByFilter extends Filters
{
    /**
     * @return array|string[]
     */
    protected array $filters = [
        'order_by'
    ];

    /**
     * @param string $fields
     * @return void
     */
    public function orderBy(string $fields): void
    {
        $params = explode(',', $fields);

        foreach ($params as $param) {
            $property = EnquiriesListOrderByFieldsEnum::tryFrom(Str::before($param,':'))?->value;
            $direction = OrderByDirectionEnum::tryFrom(Str::after($param,':'))?->value;

            if ($property && $direction) {
                if ($property === EnquiriesListOrderByFieldsEnum::FullName->value) {
                    $firstName = EnquiriesListOrderByFieldsEnum::FirstName->value;
                    $lastName = EnquiriesListOrderByFieldsEnum::LastName->value;

                    $this->builder->orderByRaw("concat($firstName,' ',$lastName) $direction");
                } else $this->builder->orderBy($property, $direction);
            }
        }
    }
}
