...
class CombinationRedirectSetupController extends Controller
{
    use Response;
    ...
    {
        try {
            $combination = Combination::withTrashed()->where('ref', $ref)->firstOrFail();
            $resource = $this->redirectDataService->addRedirect($combination, $request->validated());
            ...
            return $this->success('A redirect link has been setup for this resource.', 201, [
                'combination' => $combination->load('redirect')
            ]);
        } catch (ModelNotFoundException $e) {
            if ($request->has('model')) {
                $resource = $this->redirectDataService->addRedirectToDeletedEntity(Combination::class, $request->validated());
