...
class RegionRedirectSetupController extends Controller
{
    use Response;
    ...
    {
        try {
            $region = Region::withTrashed()->where('ref', $ref)->firstOrFail();
            $resource = $this->redirectDataService->addRedirect($region, $request->validated());
            ...
            return $this->success('A redirect link has been setup for this resource.', 201, [
                'region' => $region->load('redirect')
            ]);
        } catch (ModelNotFoundException $e) {
            if ($request->has('model')) {
                $resource = $this->redirectDataService->addRedirectToDeletedEntity(Region::class, $request->validated());
