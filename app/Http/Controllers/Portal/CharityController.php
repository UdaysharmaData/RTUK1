...
class CharityController extends Controller
{
    ...
    */
    ...
    use Response, SiteTrait, HelperTrait, UploadTrait, UploadModelTrait, DownloadTrait;
    ...
    public function __construct()
    {
        ...
        //         'edit',
        //         'update',
        //         'externalEnquiryNotifications',
        //         'completeRegistrationNotifications'
        //     ]
        // ]);
     ...
     * @queryParam active_membership bool Filter by active membership. Example: 1
    ...
    public function all(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            ...
            'active_membership' => ['sometimes', 'nullable', 'boolean'],
            'page' => ['sometimes', 'nullable', 'numeric', 'integer', 'min:1'],
            'per_page' => ['sometimes', 'nullable', 'numeric', 'integer', 'min:1'],
            'category' => ['sometimes', 'nullable', Rule::exists('charity_categories', 'ref')],
        ...
        if ($request->filled('active_membership') || AccountType::isParticipant()) {
            if ($request->active_membership) {
                $charities = $charities->where('charity_memberships.expiry_date', '>', Carbon::now())
                    ->where('charities.status', Charity::ACTIVE);
            } else {
                $charities = $charities->where(function ($query) {
                    $query->where('charity_memberships.expiry_date', '<', Carbon::now())
                        ->orWhere('charities.status', Charity::INACTIVE);
                });
            }
        }

        if ($request->filled('membership_type')) {
            $charities = $charities->where('charity_memberships.type', $request->membership_type);
        }
        ...
        $charities = $charities->orderBy('name', 'ASC');
        ...
        try {
            ...
            try {
                if ($request->filled('images')) { // Save the charity's images | TODO: Look deeper into these images upload and improve on it by adding a possibility to add an image(s) without deleting the existing ones (you may use a different route for this). Also use a route to delete an image.
                    $this->saveImages($request, $charity);
                }
    ...
    public function removeImage(int $id, int $upload_id): JsonResponse
    {
        ...
        try {
            ...
            try {
                ...
                try {
                    if ($upload && Storage::disk(config('filesystems.default'))->exists($upload->url)) { // Delete the existing image if it exists
