...
class CityRedirectSetupController extends Controller
{
    use Response;
    ...
    {
        try {
            $city = City::withTrashed()->where('ref', $ref)->firstOrFail();
            $resource = $this->redirectDataService->addRedirect($city, $request->validated());
            ...
            return $this->success('A redirect link has been setup for this resource.', 201, [
                'city' => $city->load('redirect')
            ]);
        } catch (ModelNotFoundException $e) {
            if ($request->has('model')) {
                $resource = $this->redirectDataService->addRedirectToDeletedEntity(City::class, $request->validated());
