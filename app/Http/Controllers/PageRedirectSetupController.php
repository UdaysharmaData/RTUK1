...
class PageRedirectSetupController extends Controller
{
    use Response;
    ...
    {
        try {
            $page = Page::withTrashed()->where('ref', $ref)->firstOrFail();
            $resource = $this->redirectDataService->addRedirect($page, $request->validated());
            ...
            return $this->success('A redirect link has been setup for this resource.', 201, [
                'page' => $page->load('redirect')
            ]);
        } catch (ModelNotFoundException $e) {
            if ($request->has('model')) {
                $resource = $this->redirectDataService->addRedirectToDeletedEntity(Page::class, $request->validated());
