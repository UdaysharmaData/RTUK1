...
class VenueRedirectSetupController extends Controller
{
    use Response;
    ...
    {
        try {
            $venue = Venue::withTrashed()->where('ref', $ref)->firstOrFail();
            $resource = $this->redirectDataService->addRedirect($venue, $request->validated());
            ...
            return $this->success('A redirect link has been setup for this resource.', 201, [
                'venue' => $venue->load('redirect')
            ]);
        } catch (ModelNotFoundException $e) {
            if ($request->has('model')) {
                $resource = $this->redirectDataService->addRedirectToDeletedEntity(Venue::class, $request->validated());
