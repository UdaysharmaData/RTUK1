...
use App\Services\Reporting\Traits\VenueStatsTrait;
use App\Services\Reporting\Traits\AnalyticsStatsTrait;

class VenueStatistics
{
    use SiteQueryScopeTrait, OptionsTrait, AnalyticsStatsTrait, VenueStatsTrait;

    const ENTITY = StatisticsEntityEnum::Venue;
    ...
    public static function generateStatsSummary(?int $year = null, ?string $period = null): array
    {
        ...
        $data = [
            'stats' => [self::venuesStatsData(self::ENTITY, $year, $timePeriodReferenceService)]
        ];

        $data['stats'] = array_merge($data['stats'], self::getAnalytics(Venue::class));
    ...
    public static function setParams(mixed $year, mixed $period): array
    {
        ...
        $year = in_array($year, VenueOptions::getYearOptions()->pluck('value')->toArray()) ? $year : null;
