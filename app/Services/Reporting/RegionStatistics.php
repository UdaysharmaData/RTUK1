...
use App\Services\Reporting\Traits\RegionStatsTrait;

class RegionStatistics
{
    use SiteQueryScopeTrait, OptionsTrait, AnalyticsStatsTrait, RegionStatsTrait;

    const ENTITY = StatisticsEntityEnum::Region;
    ...
    public static function generateStatsSummary(?int $year = null, ?string $period = null): array
    {
        ...
        $data = [
            'stats' => [self::regionsStatsData(self::ENTITY, $year, $timePeriodReferenceService)]
        ];

        $data['stats'] = array_merge($data['stats'], self::getAnalytics(Region::class));
    ...
    public static function setParams(mixed $year, mixed $period): array
    {
        ...
        $year = in_array($year, RegionOptions::getYearOptions()->pluck('value')->toArray()) ? $year : null;
