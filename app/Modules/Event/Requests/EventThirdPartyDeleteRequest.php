...
class EventThirdPartyDeleteRequest extends FormRequest
{
    ...
    {
        return [
            'refs' => ['required', 'array', Rule::exists('event_third_parties', 'ref')],
            'refs.*' => ['string']
        ];
    }
    ...
    public function bodyParameters()
    {
        return [
            'refs' => [
                'description' => 'The refs of the event third party(ies). Can be a string or an array of event third party(ies) refs. Must be one of '. implode(', ', EventThirdParty::inRandomOrder()
                    ->whereHas('eventCategories', function ($query) {
                        $query->whereHas('site', function ($query) {
                            $query->makingRequest();
                        });
                    })->limit(3)->pluck('ref')->toArray()),
                'example' => EventThirdParty::inRandomOrder()
                    ->whereHas('eventCategories', function ($query) {
                        $query->whereHas('site', function ($query) {
                            $query->makingRequest();
                        });
