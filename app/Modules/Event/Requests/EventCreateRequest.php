...
class EventCreateRequest extends FormRequest
{
    ...
    {
        return [
            'name' => ['required', 'string', function ($attribute, $value, $fail) { // Ensure the event does not exists for the site making the request
                $event = Event::where('name', $this->name)
                ...
                if ($event->exists()) {
                    return $fail('An event with that name already exists.');
                }

                if ($event->withTrashed()->exists()) {
                    return $fail('A [deleted] event with that name already exists. Either restore it or delete it permanently.');
                }
            }],
            'slug' => ['required', 'string', function ($attribute, $value, $fail) { // Ensure the event does not exists for the site making the request
                ...
                if ($event->exists()) {
                    return $fail('An event with that slug already exists.');
                }

                if ($event->withTrashed()->exists()) {
                    return $fail('A [deleted] event with that slug already exists. Either restore it or delete it permanently.');
                }
            }],
            ...
            'city' => [
                ...
                function ($attribute, $value, $fail) {
                    $cities = explode(',', $value); // Split the comma-separated values
                    foreach ($cities as $city) {
                        if (!City::where('ref', $city)
                            ->where(function ($query) {
                                if (!AccountType::isGeneralAdmin()) {
                                    $query->where('site_id', static::getSite()?->id);
                                }
                            })->exists()) {
                            $fail("The {$city} in {$attribute} is invalid.");
                        }
                    }
                }
            ],
            'venue' => [
                ...
                'string',
                function ($attribute, $value, $fail) {
                    $venues = explode(',', $value); // Split the comma-separated values
                    foreach ($venues as $venue) {
                        if (!Venue::where('ref', $venue)
                            ->where(function ($query) {
                                if (!AccountType::isGeneralAdmin()) {
                                    $query->where('site_id', static::getSite()?->id);
                                }
                            })->exists()) {
                            $fail("The {$venue} in {$attribute} is invalid.");
                        }
                    }
                }
            ],
            ...
            'route_info.code' => [
                'string',
                'nullable',
                // Rule::requiredIf($this->route_info && isset($this->route_info['type']) && $this->route_info['type'] == EventRouteInfoTypeEnum::EmbedCode->value),
                Rule::prohibitedIf($this->route_info && isset($this->route_info['type']) && $this->route_info['type'] != EventRouteInfoTypeEnum::EmbedCode->value)
            ],
            'route_info.media' => [
                'array',
            ...
            'route_info.media.*' => ['string', new EnsureUploadDataExists(UploadTypeEnum::Image)],
            ...
            'what_is_included.media.*' => ['string', 'string', new EnsureUploadDataExists(UploadTypeEnum::Image)],
            ...
            ...$this->eventThirdPartyCreateRequestRules(),
            ...$this->imageRules(),
            ...$this->galleryRules(),
            ...$this->metaRules(),
            ...$this->faqCreateRules(),
    ...
    public function messages() 
    {
        return [
            ...
            'route_info.code.prohibited' => 'The route info code field is prohibited when type is not embed code.',
            ...
            ...$this->eventThirdPartyCreateRequestMessages(),
            ...$this->metaMessages(),
            ...$this->faqMessages(),
            ...$this->imageMessages(),
    ...
    public function bodyParameters()
    {
        return [
            ...
            'description' => [
                'description' => 'The event description (required)',
            ...
            'region' => [
                'description' => "The ref of the region (required). Must be one of ".implode(', ', Region::inRandomOrder()->where('site_id', static::getSite()?->id)->limit(3)->pluck('ref')->all()),
            ...
            'type' => [
                'description' => "The type of the event (required). Must be one of ".implode(', ', array_column(EventTypeEnum::cases(), 'value')),
                'example' => array_column(EventTypeEnum::cases(), 'value')[0]
            ],
            'charities' => [
                'description' => "Whether the event is accessible by all charities, only the charities included or it is not accessible by excluded charities (required). Must be one of ".implode(', ', array_column(EventCharitiesEnum::cases(), 'value')),
            ...
            ...$this->eventThirdPartyCreateRequestBodyParameters(),
