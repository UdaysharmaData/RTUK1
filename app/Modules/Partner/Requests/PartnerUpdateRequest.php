...
use App\Rules\CanRoleUpdatePartner;
...
{
    use FailedValidationResponseTrait, SiteTrait, ImageValidator, GalleryValidator, MetaCustomValidator;
    ...
    {
        $siteId = Site::where('ref', $this->site)->value('id');

        return [
            'name' => ['bail', 'required_with:site', 'string', Rule::unique("partners")->where( // Ensure the name is unique for every site
                ...
                })->ignore($this->partner?->id), function ($attribute, $value, $fail) { // Check if the user has access to the site (site submitted)
                ...
                new CanRoleUpdatePartner($this->partner ?? new Partner())
            ],
            'site' => ['bail', 'required_with:name', 'string', 'exists:sites,ref', new CanRoleUpdatePartner($this->partner ?? new Partner())],
