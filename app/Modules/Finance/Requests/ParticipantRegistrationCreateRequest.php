...
class ParticipantRegistrationCreateRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        $user = [   // Validate user
            ...
            'user.email' => ['bail', 'sometimes', 'required', 'string', 'email'],
            'user.first_name' => ['sometimes', 'required', 'string'],
            'user.last_name' => ['sometimes', 'required', 'string'],
            'user.phone' => ['sometimes', 'required', 'phone:AUTO,GB'],
            'user.postcode' => ['sometimes', 'required', 'string'],
            'user.gender' => ['sometimes', 'required', new Enum(GenderEnum::class)]
        ];
        ...
        if (SiteEnum::belongsToOrganisation(OrganisationEnum::GWActive) || !app()->environment('production')) { // Only the email (user data) is collected for the RunThrough site.
            return array_merge($eec, [
                'user' => ['sometimes', 'required'],
                'user.email' => ['sometimes', 'required', 'string', 'email'],
                'user.first_name' => ['sometimes', 'required', 'string'],
                'user.last_name' => ['sometimes', 'required', 'string']
            ]);
        }
    ...
    public function messages() {
        return [
            ...
            // 'eec.*.ref.string' => [
            //     'value' => ':input',
            //     'message' => 'The selected category must be a string.'
            // ],
            // 'eec.*.ref.required' => [
            //     'value' => ':input',
            ...
            // ],
            // 'eec.*.ref.'.IsRegisteredToEEC_2::class => [
            //     'value' => ':input',
            //     'message' => ':message'
            // ],
